...
class AggregateSnapshotRepositoryImplTest {
    ...
    class WhenFindingLatest {
        ...
        @Captor
        ArgumentCaptor<SnapshotId> idCaptor;
        ...
        void findOne_givenSVUID() {
            ...
            verify(snap)
                    .getSnapshot(idCaptor.capture());

            assertThat(idCaptor.getValue())
                    .isNotNull();

            // never calculate hash, as it is given
            verify(snapshotSerializer, never())
                    .calculateProjectionClassHash(any());
        ...
        void findOne_calculatedSVUID() {
            // INIT
            Snapshot withoutSVUID = new Snapshot(new SnapshotId("some key", aggregateId), UUID
                    .randomUUID(),
                    new byte[0], false);

            when(snap.getSnapshot(any()))
                    .thenReturn(Optional.of(withoutSVUID));
