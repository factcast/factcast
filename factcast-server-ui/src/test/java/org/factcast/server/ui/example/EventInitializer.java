...
public class EventInitializer implements InitializingBean {
  ...
  public void afterPropertiesSet() throws Exception {
    fs.publish(List.of(
            Fact.builder().ns("users").type("UserCreated").version(1).aggId(USER1_AGG_ID).id(USER1_EVENT_ID)
                    .meta("hugo", "bar")
                    .build("""
                            {
                              "firstName":"Peter",
            ...
            Fact.builder().ns("users").type("UserCreated").version(1).aggId(USER2_AGG_ID).id(USER2_EVENT_ID)
                    .build("""
                            {
                              "firstName":"Werner",
                              "lastName":"Ernst",
            ...
            Fact.builder().ns("users").type("UserCreated").version(1).aggId(USER3_AGG_ID).id(USER3_EVENT_ID)
                    .build("""
                            {
                              "firstName":"Dillon",
                              "lastName":"Keller",
            ...
            Fact.builder().ns("users").type("UserCreated").version(1).aggId(USER4_AGG_ID).id(USER4_EVENT_ID)
                    .build("""
                            {
                              "firstName":"Edwin",
                              "lastName":"Jäger",
