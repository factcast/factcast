...
An event-sourced application usually performs xref:../Concept/index.adoc[two kinds of interactions] with the FactCast
server:
...
Factus API has xref:../Usage/factus/projections/types/index.adoc[explicit support for this concept].
...
xref:../Usage/factus/projections/atomicity/index.adoc[see here]) this test approach can get challenging.
...
FactCast xref:../Usage/factus/testing.adoc[provides a Junit5 extension] which starts a FactCast server
...
For details of the FactCast low-level API please refer to xref:../Usage/lowlevel/java/index.adoc[the API documentation].
...
xref:../Usage/factus/introduction.adoc[here].
...
As projection type we decided for a xref:local-managed-projection.adoc[`LocalManagedProjection`]
which is intended for self-managed, in-memory use cases. See xref:../Usage/factus/projections/types/index.adoc[here] for
