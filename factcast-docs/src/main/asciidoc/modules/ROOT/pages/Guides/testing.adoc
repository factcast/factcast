...
An event-sourced application usually performs [two kinds of interactions](xref:../Concept/index.adoc) with the FactCast
server:
...
Factus API has [explicit support for this concept](xref:../Usage/factus/projections/types/index.adoc).
...
[see here](xref:../Usage/factus/projections/atomicity/index.adoc)) this test approach can get challenging.
...
FactCast [provides a Junit5 extension](xref:../Usage/factus/testing.adoc) which starts a FactCast server
...
For details of the FactCast low-level API please refer to [the API documentation](xref:../Usage/lowlevel/java/index.adoc).
...
[here](xref:../Usage/factus/introduction.adoc).
...
As projection type we decided for a [`LocalManagedProjection`](xref:local-managed-projection.adoc)
which is intended for self-managed, in-memory use cases. See [here](xref:../Usage/factus/projections/types/index.adoc) for
