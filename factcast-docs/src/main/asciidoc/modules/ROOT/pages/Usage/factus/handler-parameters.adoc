...
Inside projections, Factus uses xref:ROOT:Usage/factus/projections/index.adoc#projections-in-general[methods annotated with `@Handler` or `@HandlerFor`]
...
|`Fact` |Provides access to all xref:ROOT:Concept/fact.adoc[Fact] details including header (JSON) and payload (JSON) |yes |yes
|`FactHeader` |the xref:ROOT:Concept/fact.adoc#the-header[Fact header]. Provides access to event namespace, type, version, meta entries and others |yes |yes
|`UUID` |the xref:ROOT:Concept/fact.adoc#the-header[Fact ID of the Fact header] |yes |yes
...
|`? extends EventObject` |an instance of a concrete class xref:ROOT:Usage/factus/introduction.adoc#eventobjects[implementing `EventObject`]. |yes |no
|===
...
|`RTransaction` |needed in a xref:ROOT:Usage/factus/projections/atomicity/redis-transactional-projections.adoc[Redis transactional projection] |yes |yes
|===
...
- xref:ROOT:Usage/factus/introduction.adoc#eventobjects[implement the `EventObject` interface] and
- provide their specification details via xref:ROOT:Usage/factus/introduction.adoc#eventobjects[the `@Specification` annotation].
