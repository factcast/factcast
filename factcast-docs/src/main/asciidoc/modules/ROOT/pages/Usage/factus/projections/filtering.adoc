...
(xref:ROOT:Usage/factus/handler-parameters.adoc[methods annotated with either `@Handler` or `@HandlerFor`])
in order to express, what the projection is interested in.

Factus will look at these methods in order to discover xref:ROOT:Concept/factspec.adoc#specification[fact specifications].
...
use the xref:ROOT:Usage/factus/projections/callbacks.adoc#postprocess[`postprocess`] callback instead.
====
