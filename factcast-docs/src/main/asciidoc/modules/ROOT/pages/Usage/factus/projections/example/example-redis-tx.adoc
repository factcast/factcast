...
_UserDeleted_ events. It solves the same problem as the example we've seen in xref:example-spring-tx.adoc[Spring transactional projections]. However, this time we use Redis as our data store and https://github.com/redisson/redisson[Redisson] as the access API.
