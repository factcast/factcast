...
a https://www.postgresql.org/docs/11/textsearch-indexes.html[global index] is used.
...
In general, xref:ROOT:Concept/index.adoc#read-subscribe[subscriptions] may consist of two possible phases:
...
Tail index rotation is configurable and described in xref:ROOT:Setup/properties.adoc#performance-reliability[the configuration properties].

[NOTE]
====
Tail indexes are implemented as https://www.postgresql.org/docs/11/indexes-partial.html[Postgres Partial Indexes].
with enabled https://www.postgresql.org/docs/11/sql-createindex.html[fastupdate].
====
...
See the xref:ROOT:Setup/properties.adoc#performance-reliability[recommendations of the configuration section] for sensible values
on the number of tail index generations.

If you encounter performance issues, see the https://www.postgresql.org/docs/11/gin-implementation.html#GIN-FAST-UPDATE[Postgres documentation] for further advice.
