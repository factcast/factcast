...
If there are only additions for instance in the new fact version, then the 'tolerant reader' can kick in and ignore unknown properties. See https://www.martinfowler.com/bliki/TolerantReader.html[Tolerant Reader]
...
For an example, see https://github.com/factcast/factcast/tree/main/factcast-examples/factcast-example-server/src/main/resources[the example registry]
