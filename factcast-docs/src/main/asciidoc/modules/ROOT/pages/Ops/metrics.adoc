...
Being a regular Spring Boot 2+ application, the FactCast Server uses https://micrometer.io[micrometer.io] as its
...
has https://micrometer.io/docs[lots of prebuilt bindings to choose from]. Please refer to the respective documentation
...
- xref:ROOT:Usage/factus/metrics.adoc[factus client library].
...
at https://github.com/factcast/factcast/blob/main/factcast-store/src/main/java/org/factcast/store/internal/StoreMetrics.java[StoreMetrics.java]
, https://github.com/factcast/factcast/blob/main/factcast-store/src/main/java/org/factcast/store/registry/metrics/RegistryMetrics.java[RegistryMetrics.java]
, https://github.com/factcast/factcast/blob/main/factcast-server-grpc/src/main/java/org/factcast/server/grpc/metrics/ServerMetrics.java[ServerMetrics.java],
or https://github.com/factcast/factcast/blob/main/factcast-server-ui/src/main/java/org/factcast/server/ui/metrics/UiMetrics.java[UIMetrics.java]
respectively.
...
|===
|operation |type |description
|publish |`timer` |Time to publish (write) a fact or a list of facts sent by the client.; Ref: xref:ROOT:Concept/fact.adoc[concepts]
|subscribe-follow |`timer` |Time to create and return a follow subscription (not the actual stream of facts).; Ref: xref:ROOT:Concept/fact.adoc[concepts]
|subscribe-catchup |`timer` |Time to create and return a catchup subscription (not the actual stream of facts).; Ref: xref:ROOT:Concept/fact.adoc[concepts]
...
|getStateFor |`timer` |Time to get the latest state token for a given fact specification. The state represents the serial of the last fact matching the specifications, and is used by the client to determine whether a fact stream has been updated at a given point in time. Relevant for optimistic locking.; Ref: xref:ROOT:Usage/factus/optimistic-locking.adoc[optimistic locking]
|publishIfUnchanged |`timer` |Time to check against the given state token and possibly publish (write) a fact or a list of facts sent by the client.; Ref: xref:ROOT:Usage/factus/optimistic-locking.adoc[optimistic locking]
|getSnapshot |`timer` |Time to read a snapshot from the cache.; Ref: xref:ROOT:Usage/factus/projections/index.adoc[snapshots]
|setSnapshot |`timer` |Time to create/update a snapshot from the cache.; Ref: xref:ROOT:Usage/factus/projections/index.adoc[snapshots]
|clearSnapshot |`timer` |Time to delete a snapshot from the cache.; Ref: xref:ROOT:Usage/factus/projections/index.adoc[snapshots]
|compactSnapshotCache |`timer` |Time to delete old entries from the snapshot cache.; Ref: xref:ROOT:Usage/factus/projections/index.adoc[snapshots]
|invalidateStateToken |`timer` |Time to invalidate the state token used for optimistic locking. The client can abort the transaction and let the server invalidate the token used for consistency.; Ref: xref:ROOT:Usage/factus/optimistic-locking.adoc[optimistic locking]
...
|catchupFact |`meter` |Counts the number of facts returned by a catchup subscription or catchup part of a follow subscription request (e.g. Factus managed projections) managed by the EventStore.; Ref: xref:ROOT:Concept/fact.adoc[concepts]
|catchupTransformationRatio |`meter` |[deprecated] Percentage of facts transformed (downcasted/upcasted) by the server in response to a subscribed client. Useful for debugging the amount of overhead due to transforming, for subscription returning a significant amount of facts.; Ref: xref:ROOT:Concept/transformation.adoc[transformation]
...
|snapshotsCompacted |`meter` |Counts the number of old snapshots deleted. This runs as a dedicated scheduled job, configured by `factcast.store.snapshot-cache-compact-cron`.; Ref: xref:ROOT:Usage/factus/projections/index.adoc[snapshots]
...
|===
|operation |type |description
|transformEvent |`timer` |Time to transform (upcast/downcast) a single fact.; Ref: xref:ROOT:Concept/transformation.adoc[transformation]
|fetchRegistryFile |`timer` |Time to retrieve a file from the schema registry.; Ref: xref:ROOT:Concept/schema-registry.adoc[facts validation]
...
the https://yidongnan.github.io/grpc-spring-boot-starter/en/[gRPC library]
that we use.
...
the https://github.com/factcast/factcast/blob/main/factcast-grpc-api/src/main/proto/FactStore.proto[protocol buffer specs].
...
- `telemetry` - used by the Guava EventBus that receives signals from the FactCast Server (
  see xref:ROOT:Ops/telemetry.adoc[telemetry])
