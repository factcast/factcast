...
While Document datastores like https://mongodb.com[MongoDB] certainly have more to offer here, PostgreSQL is surprisingly good with JSON. FactCast uses **GIN** Indexes on **JSONB** Columns in order to find matching Facts for subscriptions easily.
...
https://projects.spring.io/spring-boot/[Spring Boot] is a simple Framework to quickly spin up Java Servers. The FactCast Server is implemented using Spring Boot as a container.
