...
For full overview, you can look at the xref:changelog.adoc[changelog]
...
- high-level API called xref:../Usage/factus/index.adoc[Factus] that should make application code **MUCH** easier when using FactCast
...
- Security improvements, added Roles see xref:../Setup/gRPC Client/grpc-config-basicauth.adoc[Boot gRPC BasicAuth]
- Schema Registry tool xref:../Usage/lowlevel/cli/index.adoc[Schema Registry CLI]
- Validation of Fact payloads against JSON Schema xref:../Concept/schema-registry.adoc[Schema validation and Registry]
- Transformation between Versions (Up-/Downcasting) of Fact payloads xref:../Concept/transformation.adoc[Transformation and Registry]
...
- **Not a drop-in replacement: See xref:migration.adoc[Migration Guide]**
