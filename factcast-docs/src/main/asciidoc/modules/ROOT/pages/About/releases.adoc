...
For full overview, you can look at the xref:ROOT:About/changelog.adoc[changelog]
...
- high-level API called xref:ROOT:Usage/factus/index.adoc[Factus] that should make application code **MUCH** easier when using FactCast
...
- Security improvements, added Roles see xref:ROOT:Setup/gRPC Client/grpc-config-basicauth.adoc[Boot gRPC BasicAuth]
- Schema Registry tool xref:ROOT:Usage/lowlevel/cli/index.adoc[Schema Registry CLI]
- Validation of Fact payloads against JSON Schema xref:ROOT:Concept/schema-registry.adoc[Schema validation and Registry]
- Transformation between Versions (Up-/Downcasting) of Fact payloads xref:ROOT:Concept/transformation.adoc[Transformation and Registry]
...
- **Not a drop-in replacement: See xref:ROOT:About/migration.adoc[Migration Guide]**
...
- Note that the Jersey implementation of the REST interface has its own
  https://github.com/Mercateo/factcast-rest-jersey[place on GitHub now] and got new coordinates:
