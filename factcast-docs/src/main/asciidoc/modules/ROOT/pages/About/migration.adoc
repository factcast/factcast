...
(as documented xref:../Usage/factus/projections/snapshots/snapshot-caching.adoc[here]).
...
respective xref:redis-transactional-projections.adoc[RedisTx]
class.
...
Further documentation can be found xref:compression.adoc[here].

=== Redisson Snapshots

If you use Factus for aggregates or snapshot projections you now have to configure where to store the SnapshotCache.
Further documentation can be found xref:snapshot-caching.adoc[here]. Some properties were renamed and will be
...
You can find the current metric names xref:../Ops/metrics.adoc[here].
...
See xref:../Usage/factus/testing.adoc[testing section]
...
see xref:../Setup/gRPC Client/grpc-config-basicauth.adoc[basicauth usage]
...
There is a xref:../Usage/lowlevel/java/optimistic-locking.adoc[section on optimistic locking] as a new api feature.
