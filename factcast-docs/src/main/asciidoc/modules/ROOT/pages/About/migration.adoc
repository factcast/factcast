...
`revisionId` instead, so that revisions no longer have to be numeric; a commit hash, a date, or a semantic version work
just as well. The goal is to reduce the chance of conflicting revisions when working on projections on multiple
branches.
...
`revision` and `revisionId` are mutually exclusive. Setting both results in an `IllegalArgumentException` when the
projection is first resolved.
...
exactly the same scoped name as `revision = 1` did, so migration is stable. If you change the name during migration,
for instance from `"1"` to `"1.0"`, the key changes and the projection will be rebuilt from scratch on the next run.
====
...
bumping your FactCast dependency. The guide for 0.11.0 below explains how to add the plugin.
...
The recipe only rewrites integer literals, because only for those it can guarantee an unchanged scoped name. It
deliberately leaves these cases to you:

- a constant reference or a computed expression (`revision = MY_REVISION`, `revision = 1 + 1`), because an annotation
  attribute has to be a constant expression and there is no mechanical way to turn it into a string
...
Finally, remove the OpenRewrite plugin configuration (or just the `activeRecipes` entry) from your `pom.xml` and run
`mvn clean verify`.
...
|`spring.grpc.client.channels.factstore.address` |`spring.grpc.client.channel.factstore.target` |
...
Check
the https://github.com/spring-projects/spring-grpc/wiki/Spring-gRPC-1.1-Migration-Guide[spring-grpc migration guide]
for more details.
...
Check the https://docs.spring.io/spring-grpc/reference/appendix.html[spring-grpc documentation] for more details.
...
(as documented xref:ROOT:Usage/factus/projections/snapshots/snapshot-caching.adoc[here]).
...
respective xref:ROOT:Usage/factus/projections/atomicity/redis-transactional-projections.adoc[RedisTx]
class.
...
Further documentation can be found xref:ROOT:Setup/compression.adoc[here].

=== Redisson Snapshots

If you use Factus for aggregates or snapshot projections you now have to configure where to store the SnapshotCache.
Further documentation can be found xref:ROOT:Usage/factus/projections/snapshots/snapshot-caching.adoc[here]. Some properties were renamed and will be
automatically migrated by Spring for a while.

Ssnapshots are now stored under different keys in Redis using
a simpler
https://github.com/redisson/redisson/blob/main/redisson/src/main/java/org/redisson/client/codec/ByteArrayCodec.java[ByteArrayCodec]
codec.
...
set header =
        jsonb_set(header, '{meta}',
                  COALESCE(header -> 'meta', '{}') || concat('{"_ser":', ser,
...
You can find the current metric names xref:ROOT:Ops/metrics.adoc[here].
...
See xref:ROOT:Usage/factus/testing.adoc[testing section]
...
Details can be found https://github.com/factcast/factcast/issues/889[here].
...
see xref:ROOT:Setup/gRPC Client/grpc-config-basicauth.adoc[basicauth usage]
...
There is a xref:ROOT:Usage/factus/optimistic-locking.adoc[section on optimistic locking] as a new api feature.
...
There was a dependency upgrade of https://github.com/yidongnan/grpc-spring-boot-starter[grpc-spring-boot-starter] in
...
uses https://www.testcontainers.org/usage/database_containers.html[Testcontainers] in order to download and run an
ephemeral Postgres.
...
- Note that the jersey impl of the REST interface has its
  own https://github.com/Mercateo/factcast-rest-jersey[place on GitHub now] and got new coordinates:
