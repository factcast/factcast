...
_UserDeleted_ events. It solves the same problem as the example we've seen in xref:{{<
ref "example-spring-tx.md">}}[Spring transactional projections]. However, this time we use Redis as our data store and https://github.com/redisson/redisson[Redisson] as the access API.
...
|extended functionality which e.g. reduces I/O load. (e.g. see link:<https://www.javadoc.io/doc/org.redisson/redisson/latest/org/redisson/api/RMap.html#fastPut(K,V[`RMap.fastPut(...)`]>) and link:<https://www.javadoc.io/doc/org.redisson/redisson/latest/org/redisson/api/RMap.html#fastRemove(K...[`RMap.fastRemove(...)`].>) |standard, intuitive
