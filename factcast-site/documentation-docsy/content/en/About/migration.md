...
If you use the postgres based blacklist table:

1. The feature is guarded behind the new property `factcast.store.internalExclusionEnabled`, which defaults to `false`.
   ...
   1. For tables with more than 10,000,000 entries, the automated migration is skipped and has to be executed manually by calling:
      ...
      ```
2. To switch to the new behavior, deploy FactCast again with `factcast.store.internalExclusionEnabled` set to `true`.
...
If you use the filesystem blacklist:

1. As described above, start FactCast once without the new property `factcast.store.internalExclusionEnabled` being enabled, so that the new column can be created.
2. Use the insert statement documented [here](/concept/excluding-facts#excluding-one-or-multiple-facts), to migrate all existing entries to the new mechanism.
