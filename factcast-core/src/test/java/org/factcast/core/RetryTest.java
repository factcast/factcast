...
public class RetryTest {
	...
	void testHappyPath() throws Exception {
		...
		doThrow(new RetryableException(new IllegalStateException()))//
				...
				.doNothing()//
				.when(fs).publish(anyListOf(Fact.class));
	...
	void testMaxRetries() throws Exception {
		...
		doThrow(new RetryableException(new RuntimeException(""))).when(fs).publish(anyListOf(Fact.class));
	...
	void testThrowNonRetryableException() throws Exception {
		int maxRetries = 3;
		doThrow(new UnsupportedOperationException("not retryable")).when(fs).publish(anyListOf(Fact.class));
		FactCast uut = FactCast.from(fs).retry(maxRetries);
