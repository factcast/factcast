...
BEGIN
    ...
    WHERE c.header ->> 'ns' = p_ns
      ...
      AND (p_from_version IS NULL OR p_to_version IS NULL OR EXISTS (
          SELECT 1 FROM generate_subscripts(c.path, 1) AS positions(i)
          WHERE c.path[i] = p_from_version AND c.path[i + 1] = p_to_version
      ));
    ...
    DELETE FROM transformation_cache c
    USING unnest(keys) AS k
    WHERE c.fact_id = k.fact_id AND c.version = k.version AND c.path = k.path;
END;
$$;
