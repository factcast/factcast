...
BEGIN
    ...
    -- CONCAT_WS ignores null values and concatenates with "."
    ...
    idenditfier := REPLACE(CONCAT_WS('.', 'myvars.facttrigger', ns, type), '-', '_');

    notified := NULLIF(current_setting(idenditfier, TRUE), '');

    IF notified IS NULL THEN
        perform set_config(idenditfier, 'TRUE', TRUE);
